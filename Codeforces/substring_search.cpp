...
public:
    ...
    bool find(string &s) {
        ...
        while (l <= r) {
            ...
            int suffix = p[mid], cmp = 0;
            for (int i = 0; i < n - suffix - 1 && i < (int) s.size() && !cmp; i++) {
                if (text[suffix + i] < s[i]) cmp = -1;
                else if (text[suffix + i] > s[i]) cmp = 1;
            }

            if (!cmp && n - suffix - 1 >= (int) s.size()) return true;
